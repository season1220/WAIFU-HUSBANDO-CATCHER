...
async def rupload(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if c_type == "amv" and rarity_num != 13: await update.message.reply_text("❌ AMV ke liye **13** use karein!"); return
        if c_type == "img" and rarity_num == 13: await update.message.reply_text("❌ Photo ke liye **13** use mat karein!"); return
...
async def ball(update: Update, context: CallbackContext):
    ...
    if user.get('ball_date') != today_str:
        ...
        user['ball_count'] = 0
    if user.get('ball_count', 0) >= 6: await update.message.reply_text("🚫 Daily limit reached (6/6)."); return
...
async def gift(update: Update, context: CallbackContext):
    ...
    if not update.message.reply_to_message: await update.message.reply_text("⚠️ Reply to user: `/gift [ID]`"); return
    receiver_id = update.message.reply_to_message.from_user.id
    if sender_id == receiver_id: return
    if not context.args: await update.message.reply_text("⚠️ ID Required."); return
...
async def top(update: Update, context: CallbackContext):
    ...
    for i, u in enumerate(users, 1):
        name = escape(u['name'])
        msg += f"{i}. <b>{name}</b> ➳ {u['count']}\n"
    
    # BUTTONS
    keyboard = [
        [InlineKeyboardButton("✅ Top", callback_data="dummy"), InlineKeyboardButton("Top Group", callback_data="dummy")],
        [InlineKeyboardButton("MTOP", callback_data="dummy"), InlineKeyboardButton("Tokens", callback_data="dummy")]
    ]
    
    # SEND WITH SUKUNA IMAGE (OR SIMILAR DARK THEME)
    sukuna_img = "https://i.pinimg.com/736x/8b/6d/29/8b6d2925528065057760070742116109.jpg"
    await update.message.reply_photo(photo=sukuna_img, caption=msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
...
async def profile(update: Update, context: CallbackContext):
    ...
    msg = f"👤 <b>PROFILE</b>\n👑 Name: {name}\n💰 Gold: {bal}\n📚 Chars: {count}\n💍 Spouse: {married}\n🏰 Clan: {clan}"
    if pic.endswith((".mp4", ".gif")): await update.message.reply_video(video=pic, caption=msg, parse_mode='HTML')
    else: await update.message.reply_photo(photo=pic, caption=msg, parse_mode='HTML')
